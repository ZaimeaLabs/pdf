...
class Invoice
{
    ...
    public function addItem($name, $price, $amount = 1, $id = '-', $imageUrl = null): self
    {
        $this->items->push(Collection::make([
            ...
            'price'      => $price,
            'amount'     => $amount,
            'totalPrice' => number_format(floatval(bcmul($price, $amount, $this->decimals)), $this->decimals),
            'id'         => $id,
            'imageUrl'   => $imageUrl,
        ]));
     ...
     * @return int
     */
    private function subTotalPrice(): int
    {
     ...
     * @return mixed
     */
    private function taxPrice(Object $tax_rate = null): mixed
    {
        if (is_null($tax_rate)) {
            $tax_total = 0;
